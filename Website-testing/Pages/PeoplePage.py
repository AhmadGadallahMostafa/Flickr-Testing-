...
class PeoplePage(BasePage):
    ...
    def get_followers(self):
        ...
        list_tab.click()
        time.sleep(2)
        following_list = self.driver.find_elements(
            *PeoplePageLocators.FOLLOWER_NAME)
    ...
    def get_photos_from_following(self):
        following_photos = self.driver.find_elements(
            *PeoplePageLocators.PHOTO_BY)
        for i in range(0, len(following_photos)):
            following_photos[i] = following_photos[i].text
